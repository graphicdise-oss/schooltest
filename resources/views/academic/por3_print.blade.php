...
.doc-header td { padding: 0 3px; font-size: 13.5px; }
...
.main-table thead th { background: #fff; font-size: 10.5px; font-weight: 700; line-height: 1; }
...
.count-table { border-collapse: collapse; font-size: 12.5px; }
...
<div class="page-inner">
    ...
    <div class="doc-header">
        <table>
            <tr>
                ...
                <td style="text-align:right;white-space:nowrap;font-size:13px;">หน้า {{ $pageIdx + 1 }}</td>
            </tr>
            <tr>
    ...
    <table class="main-table">
        <thead>
            ...
            <tr>
                <th class="th-bot" style="font-size:9.5px;">เลขประจำตัวประชาชน</th>
                <th class="th-bot" style="font-size:9.5px;">เลขที่ ปพ.1:พ</th>
        ...
        <tbody>
            ...
            @endphp
            <tr class="row-top">
                <td rowspan="2" style="font-size:13px;font-weight:600;">{{ $rowNum }}</td>
                <td style="font-size:12px;">{{ $stu?->student_code }}</td>
                <td style="font-size:12px;">{{ $doc?->doc_set ?? '' }}</td>
                <td rowspan="2" style="font-size:13px;font-weight:600;">{{ $rowNum }}</td>
                <td style="font-size:12px;">{{ $stu?->thai_prefix }}{{ $stu?->thai_firstname }}</td>
                <td style="font-size:12px;">{{ $birthDayMonth }}</td>
                <td style="font-size:12px;">{{ $fatherName }}</td>
                <td rowspan="2" style="font-size:12px;">{{ $credits }}/{{ $credits }}<br><span style="font-size:11px;">{{ $gpa }}</span></td>
            ...
            <tr class="row-bot">
                <td style="font-size:11px;color:#555;border-top:0.5px dotted #999;">{{ $stu?->id_card_number }}</td>
                <td style="font-size:11px;">{{ $doc?->doc_number ?? '' }}@if($pp2?->doc_number) / {{ $pp2->doc_number }}@endif</td>
                <td style="font-size:12px;">{{ $stu?->thai_lastname }}</td>
                <td style="font-size:12px;">{{ $birthYear }}</td>
                <td style="font-size:12px;">{{ $motherName }}</td>
                <td></td>
            </tr>
            @else
            {{-- แถวว่าง --}}
    ...
    <div class="footer">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:16px;">
            <div>
                <div style="font-size:12.5px;font-weight:700;margin-bottom:3px;">จำนวนผู้สำเร็จการศึกษา</div>
            ...
            <div style="text-align:center;font-size:12.5px;min-width:190px;">
                ...
                <div style="font-size:11.5px;">{{ $approverPos }}</div>
                <div style="margin-top:4px;font-size:11.5px;">วันที่ <span style="border-bottom:0.5px solid #333;min-width:110px;display:inline-block;">{{ $approveDateFormatted }}</span></div>
            </div>
        </div>
    </div>
    @endif

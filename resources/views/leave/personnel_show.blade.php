...
<style>
    ...
    .person-info-item { display: flex; flex-direction: column; gap: 2px; }
    ...
    /* สรุปวันลา */
    .summary-chips { display: flex; flex-wrap: wrap; gap: 10px; margin: 16px 0; }
    .summary-chip {
        display: flex; align-items: center; gap: 8px;
        background: #f0f4ff; border-radius: 20px; padding: 6px 14px;
        font-size: 0.82rem; color: #5482e7; font-weight: 600;
    }
    ...
    /* ปุ่มจัดการ (วงกลมสีส้ม/แดงแบบในรูป) */
    .act-btn {
    ...
    .act-btn:hover { transform: scale(1.1); box-shadow: 0 2px 8px rgba(0,0,0,0.18); }
...
<div class="ls-page">
    ...
    {{-- การ์ดข้อมูลบุคลากร --}}
    <div class="ls-card">
        <div class="ls-icon" style="background: #5482e7;">
        ...
        <div class="person-info-grid">
            <div class="person-info-item">
                <span class="person-info-label">รหัสพนักงาน</span>
                <span class="person-info-value">{{ $personnel->employee_code ?? '-' }}</span>
            </div>
            <div class="person-info-item">
                <span class="person-info-label">แผนก</span>
                <span class="person-info-value">{{ $personnel->department ?? '-' }}</span>
            </div>
            <div class="person-info-item">
                <span class="person-info-label">ปีที่แสดง (พ.ศ.)</span>
                <span class="person-info-value">{{ $fiscal_year }}</span>
            </div>
        </div>
                ...
                @foreach ($leaveTypes as $lt)
                    ...
                    @if ($d > 0)
                        <div class="summary-chip">
                            {{ $lt->leave_type_name }}
                            <span class="chip-val">{{ number_format($d, 1) }}</span> วัน
                        </div>
                    @endif
                @endforeach
                <div class="summary-chip" style="background:#fff3e0; color:#f59e0b;">
                    รวมทั้งหมด
                    <span class="chip-val">{{ number_format($summary->sum(), 1) }}</span> วัน
                </div>
            </div>
        @endif
    </div>
                ...
                <tbody>
                    @forelse ($requests as $i => $req)
                        <tr>
                            ...
                            <td>
                                {{ \Carbon\Carbon::parse($req->request_date)->addYears(543)->format('d/m/Y') }}
                                {{ \Carbon\Carbon::parse($req->request_date)->format('H:i:s') }} น.
                            </td>
                            ...
                            <td>
                                @if ($req->requester)
                                    {{ $req->requester->thai_prefix }}{{ $req->requester->thai_firstname }} {{ $req->requester->thai_lastname }}
                                @else -
                                @endif
                            </td>
                            <td>
                                @if ($req->reviewer)
                                    {{ $req->reviewer->thai_prefix }}{{ $req->reviewer->thai_firstname }} {{ $req->reviewer->thai_lastname }}
                                @else -
                                @endif
                            </td>
                            <td>
                            ...
                            <td>
                                ...
                                <a href="{{ route('leave.requests.show', $req->id) }}"
                                   class="act-btn act-view" title="ดูรายละเอียด">
                                    <i class="fas fa-eye"></i>
                                </a>
                                {{-- พิมพ์ --}}
                                <a href="{{ route('leave.requests.print', $req->id) }}" target="_blank"
                                   class="act-btn act-print" title="พิมพ์">
                                    <i class="fas fa-print"></i>
                                </a>
                                {{-- ลบ --}}
                                <form action="{{ route('leave.requests.destroy', $req->id) }}" method="POST" class="d-inline"
                                      onsubmit="return confirm('ยืนยันการลบรายการลานี้?')">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="act-btn act-delete" title="ลบ">

...
<style>
    ...
    .bg-cyan-custom { background-color: #00bcd4; }
    .bg-orange-custom { background-color: #ff9800; }
    .card-header-text {
        margin-left: 90px; font-size: 1.25rem;
    ...
    .table > thead > tr > th {
        border-bottom: 2px solid #eee; color: #333;
        font-weight: 500; white-space: nowrap; padding-bottom: 15px;
    }
    .table > tbody > tr > td {
        vertical-align: middle; color: #555;
        border-bottom: 1px solid #f5f5f5; padding: 15px 10px;
    }
    .text-purple { color: #9b59b6; font-size: 1.1rem; }
    .text-danger-custom { color: #e74c3c; font-size: 1.1rem; font-weight: bold; }
    .btn-manage { background-color: #4caf50; color: white; border: none; font-size: 0.95rem; }
    .btn-manage:hover { background-color: #45a049; color: white; }
    ...
    .btn-toggle-pwd:hover { border-color: #00bcd4; color: #00bcd4; }
    ...
    .btn-change-pwd:hover { background: linear-gradient(135deg, #e68900, #ff9800); }
    ...
    .btn-pwd-save:hover { transform: translateY(-1px); box-shadow: 0 6px 18px rgba(0,188,212,0.35); }
    ...
    .btn-pwd-cancel:hover { background: #c0392b; }
</style>
@endpush
    ...
    <div class="floating-card mb-5">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 card-header-text">
            ...
            <div class="dropdown" style="margin-top: -15px;">
                <button class="btn btn-manage dropdown-toggle px-3 py-2 rounded-1" type="button" data-bs-toggle="dropdown">
                    จัดการข้อมูล
                </button>
                <ul class="dropdown-menu dropdown-menu-end shadow-sm">
                    <li><a class="dropdown-item" href="{{ route('personnels.create') }}"><i class="bi bi-person-plus me-2"></i>เพิ่มข้อมูลบุคลากร</a></li>
                </ul>
            </div>
        </div>
            ...
            <table class="table table-hover text-center align-middle">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $person->phone ?? '-' }}</td>
                        <td>{{ $person->date_of_birth ? \Carbon\Carbon::parse($person->date_of_birth)->addYears(543)->format('d/m/Y') : '-' }}</td>
                        <td>{{ $person->status ?? 'ปฏิบัติงาน' }}</td>
                        <td>
                            {{-- แก้ไข --}}
                            <a href="{{ route('personnels.edit', $person->personnel_id) }}" class="text-purple mx-1 text-decoration-none" title="แก้ไข">
                                <i class="bi bi-pencil-square"></i>
                            </a>

                            {{-- กุญแจ → เปิด Modal --}}
                            <a href="javascript:void(0)" class="text-purple mx-1 text-decoration-none" title="จัดการรหัสผ่าน"
                            ...
                            {{-- ลบ --}}
                            <form action="{{ route('personnels.destroy', $person->personnel_id) }}" method="POST" class="d-inline"
                                onsubmit="return confirm('ยืนยันการลบข้อมูลบุคลากรท่านนี้?');">
                                @csrf @method('DELETE')

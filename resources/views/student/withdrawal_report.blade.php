...
<style>
    ...
    .section-title {
        ...
        background: #f8faff; border-left: 3px solid #00bcd4;
    ...
    .wd-table td.left { text-align: left; }
    .wd-table td.name-col { text-align: left; color: #222; }
    ...
    @media print {
        ...
        .page { padding: 0; }
        .floating-icon, .card-header-text { display: none; }
        body { background: #fff; }
    }
</style>
@endpush
        ...
        <div style="margin-top:20px;">
            ...
            @else
                @foreach ($grouped as $sectionId => $rows)
                    @php
                        $firstRow = $rows->first();
                        $section  = $firstRow->fromSection;
                        $level    = $section?->level;
                        $sem      = $section?->semester;
                        $sectionLabel = ($level?->name ?? '-') . ' / ' . ($section?->section_number ?? '-');
                ...
                <div class="grand-summary">
                    รวมทั้งหมด {{ $withdrawals->count() }} คน
                </div>
            @endif
        </div>
    </div>

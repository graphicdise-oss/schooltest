    ...
    <div class="card2">
        ...
        @if(($dateFrom !== '' || $dateTo !== '') && $missingDateCount > 0)
            <p style="font-size:.82rem;color:#b45309;margin:14px 0 0;">
                ...
                ระบบจะยังแสดงคนกลุ่มนี้ไว้เสมอไม่ว่าจะกรองวันที่หรือไม่ เพราะไม่ทราบวันที่เข้าจริง จึงไม่สามารถตัดออกจากผลลัพธ์ได้
            </p>
        @endif
    </div>
                        ...
                        <tr>
                            <td colspan="6" class="empty">
                                ...
                                @else
                                    ...
                                    @if($levelId !== '' && $availableLevelDebug->isNotEmpty())
                                        <div style="margin-top:10px;font-size:.78rem;color:#b45309;">
                                            <i class="fas fa-triangle-exclamation"></i>
                                            คุณเลือกระดับชั้น (รหัส [{{ $levelId }}]) แต่ในข้อมูลจริงมีนักเรียนอยู่ในห้อง/รหัสเหล่านี้:<br>
                                            {{ $availableLevelDebug->implode(', ') }}<br>
                                            ถ้ารหัสที่เลือกไม่ตรงกับรายการนี้เลย แปลว่าในระบบอาจมีระดับชั้นชื่อซ้ำกัน (คนละรหัส) — ต้องแจ้งผู้ดูแลระบบให้รวม/ลบรายการซ้ำ
                                        </div>
                                    @endif
                                @endif
                            </td>

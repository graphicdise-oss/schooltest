...
class LeavePersonnelController extends Controller
{
    public function index(Request $request)
    {
        ...
        // แปลงปี พ.ศ. → ค.ศ. (ช่วงของปีนั้น)
        $yearAD = $fiscalYear - 543;
        $startAD = "{$yearAD}-01-01";
        $endAD   = "{$yearAD}-12-31";
        if ($dateFrom) $startAD = $dateFrom;
        if ($dateTo)   $endAD   = $dateTo;
        ...
        $personnels = $personnelQuery->paginate(20)->withQueryString();

        // ดึง leave summary สำหรับ personnel ที่อยู่ในหน้านี้
        ...
        // จัด index เป็น [personnel_id => collection]

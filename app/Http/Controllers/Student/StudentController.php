...
class StudentController extends Controller
{
    ...
    public function create()
    {
        ...
        $sections = ClassSection::real()->with(['level', 'semester'])->get();
        $currentSection = null;
    ...
    public function store(Request $request)
    {
        ...
        // รหัสผ่านผู้ปกครองเริ่มต้น = เลขบัตรประชาชนนักเรียน (ผู้ปกครองเปลี่ยนเองได้ภายหลัง)
        if (!empty($studentData['id_card_number'])) {
            $studentData['parent_password'] = Hash::make($studentData['id_card_number']);
        }
...
// 4. อัปเดตข้อมูลนักเรียน + ที่อยู่ (กรณีกดบันทึกซ้ำ)
    public function update(Request $request, $id)
    {
        ...
        // ซิงก์รหัสผ่านผู้ปกครองกับเลขบัตรประชาชน เฉพาะกรณีที่ผู้ปกครองยังไม่เคยเปลี่ยนรหัสผ่านเอง
        if (!empty($studentData['id_card_number']) && !$student->parent_password_changed) {
            $studentData['parent_password'] = Hash::make($studentData['id_card_number']);
        }

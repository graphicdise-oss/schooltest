...
class PersonnelController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        // ถ้ายังไม่เคยตั้งรหัสผ่านเลย (สร้างบัญชีตอนนั้นยังไม่มีเลขบัตรประชาชน) แล้วมากรอกเลขบัตรทีหลัง
        // ให้ตั้งรหัสผ่านเริ่มต้นเป็นเลขบัตรประชาชนให้อัตโนมัติเหมือนตอนสร้างครั้งแรก จะได้เข้าระบบได้
        if (empty($personnel->password) && !empty($personnelData['id_card_number'])) {
            $personnelData['password'] = Hash::make($personnelData['id_card_number']);
        }
